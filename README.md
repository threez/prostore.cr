...
**Bare strings are rejected:** `default: "CURRENT_TIMESTAMP"` is a compile error.
Use `SQL.expr("CURRENT_TIMESTAMP")` or a lambda.
...
`where` field and `order_by` field must be covered by a declared index under
